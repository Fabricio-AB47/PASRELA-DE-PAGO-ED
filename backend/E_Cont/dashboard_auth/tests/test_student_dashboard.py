...
class StudentCertificateEligibilityTests(TestCase):
    def test_certificate_is_available_for_an_approved_course(self):
    ...
    def test_approved_student_with_balance_can_download(self):
        ...
        self.assertEqual(course['certificado_estado'], 'Disponible')
    ...
    def test_preview_is_rejected_when_course_is_not_approved(self, certificate_course, build_preview):
        ...
        with self.assertRaisesRegex(StudentDashboardError, 'vista previa estará disponible'):
            preview_student_certificate({'login': 'estudiante'}, '7')
    ...
    @patch('dashboard_auth.student_dashboard.build_inscription_certificate')
    @patch('dashboard_auth.student_dashboard._student_certificate_course')
    def test_preview_is_available_after_approval_even_with_pending_payment(self, certificate_course, build_preview):
        certificate_course.return_value = (
            {'codigo_estud': '1954', 'nombre': 'Estudiante Prueba'},
            _normalize_grade_course(_grade_row(grade='8.00', paid='0.00')),
        )
    ...
    def test_backend_allows_download_when_approved_with_incomplete_payment(self, resolve_student, status, fetch_row):
        ...
        student, course = _student_certificate_course({'login': 'estudiante'}, '7')

        self.assertEqual(student['codigo_estud'], '1954')
        self.assertTrue(course['aprobado'])
        self.assertFalse(course['pago_completo'])
        self.assertTrue(course['certificado_disponible'])
